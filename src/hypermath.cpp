...
namespace hypermath
{
    ...
    {
        glm::mat3 rotate3d = glm::mat3_cast(rotation);
        glm::vec3 right = glm::normalize(rotate3d*glm::vec3(0,1,0));
        float angle = acos(glm::dot(right,glm::vec3(0,1,0)));
        rotate3d[0][0] = cos(angle);
        rotate3d[0][1] = -sin(angle);
        rotate3d[1][0] = sin(angle);
        rotate3d[1][1] = cos(angle);
        glm::mat4 result = glm::mat4();
        for(int i=0; i<2; i++)
        {
            for(int j=0; j<2; j++)
